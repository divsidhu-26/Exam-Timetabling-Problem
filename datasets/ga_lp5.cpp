...
int tot_penalty(int tt){
	...
	for(int i = 0; i < num_periods; i++ ){
		...
		if(i != 2 && i != 5 && i != 8 && i != 11 && i != 14 && i != 15 && i != 18 && i != 21 && i != 24 && i != 27 && i != 30 && i != 31 && i != 34 && i != 37 && i != 40){
			vector<string> pen;
			for(int j = 0; j < num_rooms; j++){
...
int penalty(string event, int tt, int period){
	...
	if(period < 16  && period % 3 != 0)
		previous = true;
	else if(period > 16 && period < 32 && period % 3 != 1)
		previous = true;
	else if(period > 32 &&  period % 3 != 2)
		previous = true;
	
	if(period < 15  && period % 3 != 2)
		next = true;
	else if(period > 15 && period < 31 && period % 3 != 0)
		next = true;
	else if(period > 31 && period % 3 != 1)
		next = true;
